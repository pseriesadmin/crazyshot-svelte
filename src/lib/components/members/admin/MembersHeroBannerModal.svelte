<script lang="ts">
  ...
  const MAIN_COPY_MAX = 20
  const SUB_COPY_MAX = 40
  ...
  let { initialImages, initialMode, initialMainCopy = '', initialSubCopy = '', onclose }: Props = $props()

  let images = $state<ImageItem[]>(initialImages.map((img) => ({ ...img })))
  let mode = $state<'random' | 'fixed'>(initialMode)
  let mainCopy = $state(initialMainCopy)
  let subCopy = $state(initialSubCopy)

  $effect(() => {
    images = initialImages.map((img) => ({ ...img }))
    mode = initialMode
    mainCopy = initialMainCopy
    subCopy = initialSubCopy
  })
  ...
  async function onFileChange(e: Event) {
    ...
    for (const file of files) {
      ...
      form.set('file', file)
      const res = await fetch('/api/cms/members/hero-banner', { method: 'POST', body: form })
      if (!res.ok) {
  ...
  async function removeImage(item: ImageItem) {
    ...
    fetch('/api/cms/members/hero-banner', {
      method: 'DELETE',
  ...
  async function save() {
    ...
    try {
      ...
      const { error: err } = await (supabase.rpc as any)('upsert_product_page_setting', {
        p_key: 'members_hero_banner',
        p_value: { images, mode, mainCopy, subCopy },
      })
...
<aside class="modal-panel" role="dialog" aria-modal="true" aria-label="멤버십 히어로 배너 이미지 관리">
  <div class="modal-header">
    <span class="modal-title">히어로 배너 이미지 관리</span>
  ...
  <div class="modal-body">
    <!-- 카피 문구 -->
    <div class="section">
      <p class="section-label">카피 문구 <span class="hint">히어로 배너 위 텍스트</span></p>
      <div class="copy-preview-box">
        <label class="copy-field-label" for="hero-main-copy">
          메인카피 <span class="copy-count" class:over={mainCopy.length > MAIN_COPY_MAX}>{mainCopy.length}/{MAIN_COPY_MAX}</span>
        </label>
        <input
          id="hero-main-copy"
          type="text"
          class="copy-input copy-input-main"
          maxlength={MAIN_COPY_MAX}
          bind:value={mainCopy}
          placeholder="메인카피를 입력하세요"
        />

        <label class="copy-field-label" for="hero-sub-copy">
          서브카피 <span class="copy-count" class:over={subCopy.length > SUB_COPY_MAX}>{subCopy.length}/{SUB_COPY_MAX}</span>
        </label>
        <textarea
          id="hero-sub-copy"
          class="copy-input copy-input-sub"
          maxlength={SUB_COPY_MAX}
          rows="2"
          bind:value={subCopy}
      ...
      <div class="section">
        ...
        <CmsDragList bind:items={images} itemKey={(item) => item.path}>
          {#snippet renderItem(item)}
            <div class="img-row">
              <img src={item.url} alt="히어로 배너" class="img-thumb" />
    ...
    {:else}
      <p class="empty-msg">이미지를 추가하면 히어로 배너로 사용됩니다.<br />이미지가 없으면 기본 이미지가 사용됩니다.</p>
    {/if}
...
<style>
  ...
  /* 카피 문구 미리보기 박스 — 실제 히어로 다크 배경 위 화이트 텍스트를 그대로 재현 */
  .copy-preview-box {
    background: var(--cs-dark);
  ...
  .copy-field-label:first-child { margin-top: 0; }

  .copy-count { font-weight: 400; }
  .copy-count.over { color: var(--cs-red-badge); }

  /* cms-field 골격(패딩·반경·포커스링) + 카피 전용 폰트토큰·화이트컬러 */
  .copy-input {
    width: 100%;
    min-height: 44px;
    padding: 12px 16px;
  ...
  .copy-input:focus { outline: 2px solid var(--cs-purple); outline-offset: -2px; }
  .copy-input::placeholder { color: rgba(255, 255, 255, 0.4); }

  /* 메인카피: --text-m-ad-kr-30(모바일) / --text-pc-ad-kr-60(PC) */
  .copy-input-main {
    font: var(--text-m-ad-kr-30);
  }
  @media (min-width: 1024px) {
    .copy-input-main { font: var(--text-pc-ad-kr-60); }
  }

  /* 서브카피: --text-m-body-16B(모바일) / --text-pc-title-18(PC) */
  .copy-input-sub {
    font: var(--text-m-body-16B);
  }
  @media (min-width: 1024px) {
    .copy-input-sub { font: var(--text-pc-title-18); }
  }
