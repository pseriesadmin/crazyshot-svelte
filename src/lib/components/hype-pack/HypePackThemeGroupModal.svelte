<script lang="ts">
  ...
  let { groups, packageCategoryKey = null, onclose }: Props = $props()
  ...
  async function uploadImage(grp: LocalGroup): Promise<string> {
    ...
    const path = `hype-pack-theme-groups/${grp._tempId}-${Date.now()}.${ext}`
    ...
    try {
      ...
      for (const oid of origIds) {
        if (!currentIds.has(oid)) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const { data: res } = await (supabase.rpc as any)('cms_delete_hype_pack_theme_group', { p_id: oid }) as { data: { ok: boolean; error?: string } }
      ...
      for (let i = 0; i < withUrls.length; i++) {
        ...
        if (g.id === null) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const { data: res } = await (supabase.rpc as any)('cms_create_hype_pack_theme_group', params) as { data: { ok: boolean; error?: string } }
          if (!res?.ok) throw new Error(res?.error ?? '생성 실패')
        } else {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const { data: res } = await (supabase.rpc as any)('cms_update_hype_pack_theme_group', { p_id: g.id, ...params }) as { data: { ok: boolean; error?: string } }
...
<div class="tg-panel" role="dialog" aria-modal="true" aria-label="테마그룹 관리">
  <!-- 헤더 -->
  <div class="tg-header">
    <span class="tg-header-title">Pack 테마그룹 관리</span>
...
<style>
  ...
  /* 썸네일 버튼 — 정사각 라운드(2026-08-25 확정, 원형에서 변경) */
  .tg-thumb-btn {
    flex-shrink: 0;
    width: 48px;
    height: 48px;
