...
export interface ApplyTemplateOptions {
  ...
  /**
   * 작성 모드 — 'flow' | 'canvas' | 'spreadsheet' | 'html'.
   * canvas / spreadsheet / html 템플릿 적용 시 반드시 해당 모드를 전달해야 contracts 테이블에 정확히 기록됨.
   * 미전달 시 서버 기본값(flow)이 유지되므로 canvas/spreadsheet/html 발행 경로는 반드시 명시할 것.
   */
  authoring_mode?: 'flow' | 'canvas' | 'spreadsheet' | 'html'
  /**
   * 캔버스 문서 (canvas 모드 전용).
  ...
  /**
   * HTML 고정 템플릿 문서 (html 모드 전용).
   * authoring_mode='html'일 때 변수 치환이 완료된 HTML 문자열을 전달해야
