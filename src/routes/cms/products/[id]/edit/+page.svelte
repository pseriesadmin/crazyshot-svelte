<script lang="ts">
  ...
  function linkToSelected(link: ProductOptionLinkRow): SelectedOption {
    return {
      option_product_id: link.option_product_id,
      name: link.option_product_name,
      price_24h: link.price_24h ?? 0,
  ...
  let selectedOptions = $state<SelectedOption[]>(
    (data.optionLinks ?? []).map(linkToSelected)
  )
  ...
  async function searchOptionProducts() {
    ...
    const { data: pdata, error: err } = await supabase
      .from('products')
                ...
                {#each optionResults as item (item.id)}
                  <li class="option-result-item">
                    ...
                    <div class="option-result-info">
                      ...
                      <a href="/products/{item.id}" target="_blank" rel="noopener noreferrer" class="btn-detail-link">상세정보 더보기</a>
                    </div>
            ...
            <div class="selected-option-card">
              ...
              <div class="selected-option-info">
                ...
                <div class="selected-option-cbs">
                  <label class="cb-label">
                    <input type="checkbox" class="cb-input" bind:checked={selectedOptions[i].is_required} />
                    필수 선택
                  </label>
                  <label class="cb-label">
                    <input type="checkbox" class="cb-input" bind:checked={selectedOptions[i].min_select_required} />
                    최소 1개 선택 필수
                  </label>
                  <label class="cb-label">
                    <input type="checkbox" class="cb-input" bind:checked={selectedOptions[i].delivery_rental_disabled} />
                    배송 대여 불가
                  </label>
                </div>
              </div>
