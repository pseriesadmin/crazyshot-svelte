...
export const GET: RequestHandler = async ({ params, locals, url }) => {
  ...
  const { data, error } = await admin
    .from('contracts')
    .select('title, content_blocks, specifications, authoring_mode, canvas_document, spreadsheet_document, html_document, html_issuer_signature_url, html_issuer_signature_width, html_issuer_signature_offset_x, html_issuer_signature_offset_y, contract_terms_text, privacy_terms_text')
    .eq('id', contractId)
    .maybeSingle()
...
export const PATCH: RequestHandler = async ({ params, locals, request }) => {
  ...
  // Migration #478 — 예약별 1회성 "계약조항"·"개인정보동의" 원문(특약 specifications와
  // 동일 성격). 미지정 필드는 기존 값 보존, null 전송 시 "기본 문구로 되돌리기"로 해석.
  if ('contract_terms_text' in body) {
    updatePayload.contract_terms_text = body.contract_terms_text ?? null
  }
  if ('privacy_terms_text' in body) {
    updatePayload.privacy_terms_text = body.privacy_terms_text ?? null
  }
