...
// 아바타는 이미지 전용 — 공통 UPLOAD_ACCEPTED_TYPES 중 PDF 제외
const AVATAR_ACCEPTED_TYPES = ['image/png', 'image/jpeg', 'image/webp', 'image/heif', 'image/heic'] as const
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!AVATAR_ACCEPTED_TYPES.includes(file.type as (typeof AVATAR_ACCEPTED_TYPES)[number])) {
    return json({ ok: false, error: 'PNG, JPEG, WebP, HEIF 이미지 파일만 업로드할 수 있어요.' }, { status: 400 })
  }
