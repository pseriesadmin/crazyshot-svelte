<script lang="ts">
  ...
  const KEYWORDS = ['SONY', 'CANON', 'NIKON', 'Fujitsu', 'Olympus', 'Panasonic']
  ...
  const D_PER_PAGE  = 3
  const D_MAX_PAGE  = Math.ceil(desktopSlides.length / D_PER_PAGE) - 1  // = 1

  let activeCategory = $state(1)  // Camera default
  let activeCategoryLabel = $derived(CATEGORIES.find(c => c.id === activeCategory)?.label ?? '')
  let dPage = $state(0)

  let visibleSlides = $derived(desktopSlides.slice(dPage * D_PER_PAGE, dPage * D_PER_PAGE + D_PER_PAGE))
  ...
  <div class="body-wrap">
      ...
      <div class="cat-icons">
        {#each CATEGORIES as cat}
          ...
          >
            ...
            <span class="cat-label" class:active={activeCategory === cat.id}>{cat.label}</span>
          </button>
        {/each}
      </div>
    ...
    <div class="d-slider-outer">
      <div class="d-slider-relative">
        ...
        <!-- Cards -->
        <div class="d-slider-cards">
          {#each visibleSlides as slide, i (dPage * 3 + i)}
            <a href="/products/9" class="d-feat-card">
              <div class="d-feat-bg"></div>
              <div class="d-feat-img-box">
                <img
                  src={slide.img} alt={slide.name} class="abs-img"
                  style="height:{slide.imgStyle.h};left:{slide.imgStyle.l};top:{slide.imgStyle.t};width:{slide.imgStyle.w}"
                  loading="eager"
                />
              </div>
              <div class="d-feat-info">
                <p class="d-feat-price">{slide.price}</p>
                <div>
                  <p class="d-feat-name">{slide.name}</p>
                  <p class="d-feat-desc">{slide.desc}</p>
                </div>
              </div>
            </a>
          {/each}
...
<style>
  ...
  /* Desktop category indicator dots – hidden on mobile */
  ...
  .m-slider-track {
    ...
    padding: 0 25px;
    /* compensate for body-wrap's own 25px left padding */
    margin-left: -25px;
    margin-right: -25px;
  }
  ...
  .d-slider-relative {
    position: relative;
    /* space for the absolute-positioned side buttons */
  }
  ...
  .d-nav-btn:hover:not(:disabled) { background: #1d183e; }
  .d-nav-btn:disabled { opacity: 0.3; cursor: not-allowed; }
  .d-nav-prev { left: -56px; }
  .d-nav-next { right: -56px; }
