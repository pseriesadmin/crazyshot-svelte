-- Migration #138: get_customer_list 전체 복원 + 문서 URL 필드 추가
-- migration #135에서 누락된 identity_type / identity_doc_url / identity_verified_at / password_set 복원
-- 신규: foreign_doc_url, foreign_verified_at 추가
-- COALESCE로 기존 student_doc_url / student_verified_at 하위 호환 유지
-- 2026-07-22
...
BEGIN
  ...
  SELECT
    ...
    -- 신규 컬럼 우선, 없으면 기존 student_doc_url 폴백
    COALESCE(up.identity_doc_url, up.student_doc_url)::TEXT    AS identity_doc_url,
    COALESCE(up.identity_verified_at, up.student_verified_at)  AS identity_verified_at,
