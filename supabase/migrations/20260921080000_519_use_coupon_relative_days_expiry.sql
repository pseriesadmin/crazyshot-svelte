-- Migration 519: use_coupon — relative_days 유효기간 만료 체크 추가
--
-- 배경: relative_days 쿠폰 모드 소진 처리 시, 기존 valid_until 기반 만료 체크가
--   first_viewed_at + valid_days 기준 분기를 인식하지 못해 relative_days 쿠폰을
--   무기한 유효(unlimited 취급)로 처리하는 결함이 예상됨.
--
-- 수정: Migration 511(use_coupon)을 기반으로 두 지점만 변경:
--   1) SELECT 목록에 uc.first_viewed_at, c.validity_type, c.valid_days 추가
--   2) 만료 체크를 validity_type 기반 분기로 교체
--      - relative_days: first_viewed_at IS NOT NULL AND valid_days IS NOT NULL
--                       AND (first_viewed_at + valid_days || ' days')::INTERVAL) < now()
--        → first_viewed_at이 아직 NULL이면 카운트다운 미시작 → 만료 없음(unlimited 취급)
--      - 그 외(fixed_period/unlimited): 기존 valid_until 체크 그대로 유지
--   ②③④ 하위 로직(주문의존 자격조건, 사용자의존 자격조건, 소진 처리) 전부 무변경.
--
-- 시그니처 변경 없음: CREATE OR REPLACE 적용 가능.
-- 롤백: Migration 511의 use_coupon 정의로 되돌릴 것.
...
BEGIN
  ...
  IF v_uc.validity_type = 'relative_days' THEN
    -- "첫 확인일로부터 N일" 모드:
    --   first_viewed_at + valid_days < now() 이면 만료
    --   first_viewed_at이 아직 NULL(카트 미진입 등 비정상 경로) → 만료 없음(unlimited 취급)
    ...
    END IF;
  ELSE
    -- fixed_period / unlimited: 기존 valid_until 만료 체크 그대로 유지
  ...
  THEN
    ...
    -- applicable_categories 체크 — type='category'인 쿠폰만 실제 게이팅
  ...
  -- 1인당 사용 한도 — 이 사용자가 이미 이 쿠폰(coupon_id)을 몇 번
