-- Migration 402 — cancel_reservation_payment v_cancelled_ids 실제 전환 확인 수정 (2026-08-31)
-- 목적(RSV-B-C2): Migration 384에서 PERFORM update_reservation_status(v_rid, 'cancelled') 후
--   반환값을 버리고 무조건 v_cancelled_ids에 추가해, 실제로 'cancelled'로 전환되지 않은
--   예약(completed·returned 등 터미널 상태)도 알림 대상에 포함되는 버그 수정.
--   수정: PERFORM 후 실제 status를 재조회해 'cancelled'인 경우에만 v_cancelled_ids에 포함.
--   환불(Toss 결제취소) 자체는 주문 전체 그대로 — 알림 대상만 좁힘(GATE B Q2(a) 확정).
...
DECLARE
  ...
  v_actual_status       TEXT;  -- RSV-B-C2: PERFORM 후 실제 status 재조회용
BEGIN
  ...
  -- RSV-B-C2 수정: PERFORM 후 실제 status를 재조회해 'cancelled'인 경우에만
  -- v_cancelled_ids에 포함 — completed·returned 등 터미널 상태는 update_reservation_status가
  -- no-op 또는 거부할 수 있으므로 반환값 버림이 아닌 재조회로 실제 전환 여부를 확인한다.
  ...
  FOREACH v_rid IN ARRAY v_all_reservation_ids LOOP
    ...
    IF v_current_status IS NOT NULL AND v_current_status <> 'cancelled' THEN
      ...
      -- ★ RSV-B-C2 핵심 수정: PERFORM 후 실제 status 재조회
      SELECT status INTO v_actual_status
      FROM rental_reservations
      WHERE id = v_rid;

      -- 실제로 'cancelled'로 전환된 경우에만 알림 대상에 포함
      IF v_actual_status = 'cancelled' THEN
...
COMMENT ON FUNCTION public.cancel_reservation_payment(BIGINT, UUID, TEXT) IS
  'RSV-B-C2 수정(Migration 402): PERFORM update_reservation_status 후 실제 status 재조회해
   실제로 cancelled로 전환된 예약만 v_cancelled_ids에 포함(알림 대상 정확성 확보).
   환불(Toss 결제취소) 자체는 주문 전체 금액 그대로 — 알림만 좁힘(GATE B Q2(a)).';
