...
□ [2026-09-05 신설] 이 UI 요소가 표시하려는 데이터(가격·배지·상태 등)를 이미 다른 화면이
  보여주고 있지 않은가? → grep으로 프로젝트 전체(특히 cart↔products/[id]처럼 같은 도메인
  개념을 공유하는 화면쌍)에서 유사 클래스명·마크업을 먼저 검색한다. 발견되면 그 화면의
  실제 계산된 스타일 값(색상·폰트크기·구조)을 그대로 이식한다 — "이 파일 자체 관례"라는
  이유로 새 팔레트·구조를 만들지 않는다. 클래스명만 이 파일 접두사를 따르고 값은 100% 동일하게.
```
...
□ 다른 화면에 이미 확정된 정본 UI 패턴(색상·구조)이 있는데 grep 없이 새 팔레트를 임의
  창작 — cart 옵션카드(.dual-price-row--opt, red-badge)가 정본인데 products/[id] 옵션
  이중가격을 회색(--cs-text-mid/--cs-text-dark)·다른 DOM구조로 새로 만듦
  (uiux-index.md, 2026-09-05 옵션 이중가격 사례 — Stephen 직접 지적 후 정정)
```
