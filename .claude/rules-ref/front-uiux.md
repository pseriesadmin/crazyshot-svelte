...
/* 반응형 크기 — 비율 3:2 자동 유지 (Mobile 기본 → PC 축소, 위 §17 표와 동일 방향) */
.checkbox-btn-terms svg { width: 22px; height: 15px; }
@media (min-width: 768px) {
  .checkbox-btn-terms svg { width: 18px; height: 12px; }
}
...
### 수직정렬 광학보정 (2026-09-11 추가)

> **"체크아이콘 수평/수직 중앙정렬 맞춰줘" 또는 "체크아이콘이 텍스트보다 떠 보인다" 언급 시
> → 재도출 없이 아래 보정을 즉시 적용.**

체크마크 도형은 위쪽 두 갈래(양 끝)가 아래쪽 꼭짓점 1개로 모이는 비대칭 형태라, `flex
items-center`로 아이콘 박스를 텍스트와 기하학적으로 중앙정렬해도 시각적으로는 아이콘이
텍스트보다 살짝 위로 떠 보인다(도형 무게중심이 세로 중심보다 위쪽에 몰려 있기 때문 —
아래로 갈수록 한 점으로 좁아지는 형태의 공통적인 광학 착시). 이를 보정하려면 그 화면의
버튼 전용 클래스로 범위를 한정해 `translateY`로 살짝 내려준다 — **전역 `.checkbox-btn-terms`
규칙에 직접 추가 금지**(다른 사용처, 예: 체류기간 선택 콤보 버튼까지 함께 밀려 내려가 그
쪽 정렬이 오히려 깨진다).

```css
/* 예: 텍스트 옆에 단독 배치되는 약관 동의 버튼(.consent-btn)에만 스코프 */
.consent-btn .checkbox-btn-terms { transform: translateY(2px); }
```

실사용 예: `ProfileTabContent.svelte`의 "[옵션] 개인정보 수집 및 이용 동의"/"[옵션] 개인정보
제 3자 제공" 동의 항목 버튼(`.consent-btn`) — Stephen이 실화면에서 "체크 아이콘이 텍스트에
비해 좀 더 아래로 내려와야 할 것"으로 지적해 확정.
...
[ ] 텍스트와 나란히 단독 배치되는 용법이라면 수직정렬 광학보정(translateY, 그 화면
    전용 클래스로 스코프)을 적용했는가? 전역 .checkbox-btn-terms를 직접 건드리지
    않았는가?
```
...
## 19. 모바일 반응형 화면 미세떨림(100vh 재계산) — PC 브라우저 모바일 시뮬레이션 전용 결함 ★★★

> **"모바일 반응형 화면 미세떨림" 언급 시 → 원인 재추론 없이 아래 절차를 즉시 실행.**
> Stephen이 이 문구를 쓰면 "PC 브라우저 개발자도구 모바일 반응형(디바이스 툴바) 모드로
> 화면을 스크롤할 때, 페이지 전체가 미세하게 흔들리는" 그 특정 증상을 가리키는 것으로
> 간주하고, 아래 원인·수정법을 바로 적용한다 — 스크롤 리스너나 애니메이션 쪽을 다시
> 의심하지 말 것(그건 §전 GNB·바텀탭 스크롤 인터랙션 관련 별개 이슈다, `ui-mobile.md` 참고).

### 19-1. 원인

모바일 브라우저(및 PC 브라우저의 모바일 디바이스 시뮬레이션)는 스크롤 시 주소창/툴바가
숨었다 나타났다 하며 실제 뷰포트 높이(`window.innerHeight`)가 그때마다 바뀐다. CSS
`100vh`는 이 변동을 그대로 반영해 재계산되므로, 페이지 최상위 컨테이너가
`min-height: 100vh`를 쓰면 스크롤할 때마다 그 컨테이너의 높이가 미세하게 늘었다 줄었다
하면서 전체 레이아웃이 잔떨림한다 — 잘 알려진 "모바일 100vh 버그"다. `100dvh`(dynamic
viewport height)는 이 재계산에 반응하지 않도록 브라우저가 최적화해주는 단위라, 최상위
컨테이너에 한해 `100vh`를 `100dvh`로 바꾸는 것만으로 해결된다.

### 19-2. 수정 절차 (자동 적용)

```
1. 프로젝트 전역에서 다음 패턴을 검색:
     grep -rn "min-height:\s*100vh" src/routes src/lib/components
2. 검색된 각 위치가 "페이지 최상위 루트 컨테이너"(예: .cart-root · .page-root · .page 등,
   보통 <style> 블록 최상단 셀렉터)인지 확인 — 페이지 전체 높이를 잡는 용도가 맞다면
   전부 100vh → 100dvh로 교체.
   ⛔ 주의: 부분 요소(모달 내부 스크롤 영역, 카드 높이 등 "화면 전체 높이"가 아닌 국소적
   100vh 사용)까지 무분별하게 바꾸지 말 것 — 반드시 "페이지 최상위 루트"인지 먼저 판단.
3. 교체한 자리에 "임의로 100vh로 되돌리지 말 것" 방지 주석을 남긴다(아래 패턴 참고).
4. `.cart-footer`류 "고정 하단바" 요소의 `padding-bottom: env(safe-area-inset-bottom)`는
   이 100vh/100dvh 이슈와 무관한 별개 패턴이니 건드리지 않는다.
```

```css
/* ✅ 표준 수정 패턴(그대로 복붙) */
.page-root {  /* 또는 .cart-root · .page 등 그 화면의 최상위 컨테이너 셀렉터 */
  /* PC 반응형 모바일 시뮬레이션에서 스크롤 시 브라우저 툴바 숨김/재출현으로 100vh 값이
     매번 재계산돼 화면이 미세하게 잔떨림하는 잘 알려진 모바일 100vh 버그 — 100dvh로
     고정. 임의로 100vh로 되돌리지 말 것. */
  min-height: 100dvh;
  /* ...나머지 기존 선언 유지... */
}
```

### 19-3. 이미 적용(및 회귀 이력이 있는) 화면 — 우선 점검 대상

| 화면 | 파일 | 셀렉터 |
|---|---|---|
| 마이페이지 프로필 | `src/routes/account/profile/+page.svelte` | `.page-root` |
| 계약완료 | `src/routes/contract/complete/+page.svelte` | `.page` |
| 계약만료 | `src/routes/contract/expired/+page.svelte` | `.page` |
| 장바구니 | `src/routes/cart/+page.svelte` | `.cart-root` |

> 이 4곳은 2026-08-26 최초 확정(account/profile에서 발견 후 contract 2곳·cart로 확산
> 적용) 이후 **최소 2곳(cart·account/profile)이 2026-09-02 재확인 시점에 다시 100vh로
> 회귀돼 있었다** — 병렬 세션 편집·리팩터 과정에서 조용히 되돌아갈 수 있는 항목이니,
> 이 명령이 들어오면 표에 없는 새 화면까지 포함해 전역 grep으로 매번 처음부터 다시
> 확인할 것(표만 믿고 스킵하지 말 것).

### GATE C 확인 항목

```
[ ] `grep -rn "min-height:\s*100vh" src/routes src/lib/components`로 전역 재검색했는가?
[ ] 검색된 각 위치가 "페이지 최상위 루트"가 맞는지 확인 후에만 100dvh로 교체했는가?
    (모달·카드 등 국소 100vh까지 무분별하게 바꾸지 않았는가)
[ ] 교체 위치에 "임의로 100vh로 되돌리지 말 것" 방지 주석을 남겼는가?
[ ] `.cart-footer`류 고정 하단바의 `env(safe-area-inset-bottom)`는 건드리지 않았는가?
[ ] svelte-check 신규 에러 0건 확인했는가?
```

---

## 20. 대표 BI(로고) 표준 ★★★ (2026-08-20 확정, 2026-09-02 복원)

> ⚠️ 이 섹션은 2026-08-20에 최초 §19로 작성됐으나, 2026-09-02 다른 세션이 같은 번호(§19)로
> "모바일 반응형 화면 미세떨림" 섹션을 신설하면서 내용이 유실됐다(sp3-qa-agent 검수로 발견).
> 번호 충돌을 피하기 위해 §20으로 재작성한다 — 신규 섹션 추가 시 반드시 파일 끝의 기존
> 번호를 확인하고 그 다음 번호를 쓸 것(동일 실수 재발 방지).

### 20-1. 두 개의 로고 자산 계열 — 절대 혼동 금지

프로젝트에는 완전히 다른 두 SVG 자산 계열이 존재한다. 재사용 가능한 `Logo` 컴포넌트는 없고
양쪽 다 완전히 하드코딩된 인라인 SVG(또는 정적 파일 참조)다.

| 구분 | ① drop-shadow 정본 BI | ② 평면 로고 |
|---|---|---|
| 형태 | 인라인 SVG, `filter0_d_*` 드롭섀도 필터 포함 | 정적 파일 `/static/logo-bi2.svg` (`<img>` 참조) |
| 색상 | 빨강 `#CF0000` + 흰색 `#FEFEFF` | (별도 확인 필요 — 이 문서 범위 아님) |
| 적용처 | 전역 푸터(`src/routes/+layout.svelte`), `/contract/[token]`(고객 서명 화면),
`/account/rental/[id]/contract`(고객 계약 열람 화면) | GNB·`MobileMoreMenu`·CMS 레이아웃 등 |
| 용도 | **"공식 문서/거래 성격이 강한 화면"**(계약서·서명·영수 등)의 헤더 | 내비게이션·목록형 화면의 헤더 |

**신규 화면 헤더에 로고를 넣어야 할 때 판단 기준**: 그 화면이 "계약서 열람·서명·인쇄"처럼
공식 문서 성격이 강하면 ① drop-shadow 정본 BI를, 일반 내비게이션·목록·설정 화면이면 기존
관례대로 GNB류의 ② 평면 로고를 따른다. 애매하면 가장 최근에 만들어진 동일 성격 화면
(현재는 `/account/rental/[id]/contract`)의 코드를 그대로 복제한다.

### 20-2. ① drop-shadow 정본 BI — 재사용 방법

새 화면에 필요할 땐 `src/routes/account/rental/[id]/contract/+page.svelte`(또는
`src/routes/contract/[token]/+page.svelte`)의 `.logo-svg` SVG 마크업 전체를 그대로 복사한다.

```
<svg class="logo-svg" width="71" height="40" viewBox="0 0 117 66" fill="none" aria-label="CRAZYSHOT" role="img">
  <g filter="url(#filter0_d_XXX)">...</g>
  <defs><filter id="filter0_d_XXX" ...>...</filter></defs>
</svg>
```

⛔ **filter id는 같은 페이지에 이 SVG가 2개 이상 동시 렌더될 가능성이 없더라도 반드시
소스 파일과 다른 접미사로 바꿀 것**(예: 원본이 `filter0_d_825_3724`면 복제본은
`filter0_d_825_3724_a`) — SVG `id`는 문서 전역 네임스페이스라 우연히 같은 값을 가진 다른
SVG가 같은 페이지에 존재하게 되면 필터가 깨질 수 있다.

### 20-3. `--cs-orange` 오독 방지 각주

디자인 토큰 `--cs-orange`(`#FF4500`)는 "로고 전용, 버튼 금지"라는 **사용 제약**을 나타내는
토큰일 뿐, **실제 BI 로고의 색상이 아니다.** 위 20-1의 드롭섀도 정본 BI는 오렌지를 전혀 쓰지
않고 빨강(`#CF0000`)+흰색을 쓴다. "로고=오렌지"로 오독해 신규 화면에 `--cs-orange` 텍스트
로고(`CRAZY` + `<span style="color:var(--cs-orange)">SHOT</span>` 같은 패턴)를 새로 만들지
말 것 — 실제 발견된 결함(2026-08-20)이 정확히 이 오독 패턴이었다(신규 계약 열람 화면에
텍스트 로고가 방치돼 있었음, drop-shadow SVG로 교체 완료).

### GATE C 확인 항목

```
[ ] 신규 화면 헤더에 로고가 필요할 때 §20-1 판단기준(공식문서성 vs 내비게이션)으로 어느
    계열을 쓸지 먼저 판단했는가?
[ ] drop-shadow 정본 BI를 복제할 때 filter id를 소스와 다르게 바꿨는가?
[ ] `--cs-orange`를 "로고 색상"으로 오인해 텍스트 로고를 새로 만들지 않았는가?
```
...
*2026-09-02 §19 추가 — "모바일 반응형 화면 미세떨림"(100vh 재계산 잔떨림) 트리거 명령
신설. 2026-08-26 최초 확정된 100vh→100dvh 패턴이 cart·account/profile 2곳에서 재차
...
*2026-09-11 §17 갱신 — "수직정렬 광학보정" 절 신설(체크마크가 아래 꼭짓점으로 모이는
비대칭 형태라 flex 중앙정렬만으로는 텍스트보다 위로 떠 보이는 착시 보정, `translateY`를
그 화면 전용 버튼 클래스로 스코프하는 원칙 + GATE C 체크항목 1건 추가). 실사용 사례:
`ProfileTabContent.svelte` 개인정보 동의 버튼(`.consent-btn`). 같은 절 "표준 사용 패턴"
