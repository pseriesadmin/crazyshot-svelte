...
# 소스: CMS-Design-System-Tokens.json (2026-07-08) + 페이지 실측 + 주메뉴 코드 분석 (2026-07-19)
# 최종 업데이트: 2026-07-19
...
| 행 간격 (gap) | `8px` (소) / `15px` (중) / `20px` (대) — 2026-08-15 확정 (구 12px/16px 폐기) |

> ⚠️ **행 간격(gap) 소/중/대 값은 2026-08-15 재확정됨** — 이전 값(소 8px/중 12px/대 16px)은
> 더 이상 유효하지 않다. 이 소/중/대 스케일은 목록의 `gap`에 적용한다(예: `_FormatTab.svelte`
> `.fmt-list` — `gap: 20px`(대)). 카드 내부 패딩(위 `카드 내부 패딩` 행, 20px/28px)은 `.panel`
> 등 별도 컨텍스트의 기존 값이라 이번 재확정과 무관하게 유지한다.
...
### 7-7-2. 목록 제안 피커 (`SuggestPicker`) — 로컬 옵션 공통 ★

> **용도**: `<select>` 대체 — 페이지 로드 옵션을 유사이름 제안 UI로 선택  
> **컴포넌트**: `src/lib/components/common/SuggestPicker.svelte` ← **공통 컴포넌트 (CMS·USER 공유)**  
> **타입**: `src/lib/types/suggest-picker.ts`  
> **최초 적용**: `products/new` 조합그룹 선택  
> ⚠️ `CmsSuggestPicker` 경로는 shim으로 유지되나 신규 작성 시 반드시 공통 경로 사용
...
> **최초 적용**: `products/new` 조합그룹 선택 + `ProductCategoryModal.svelte` 카테고리·키워드 추가
> ⚠️ 구경로 `cms/CmsSuggestPicker` / `cms-suggest-picker` 는 re-export shim — 신규 작성 시 공통 경로 사용

### 12-0. variant prop — 2종 기본 모드

| `variant` | 용도 | `listLabel` 기본값 |
|---|---|---|
| `'category'` | 분류(카테고리) 선택 — `meta` 2줄(이름+코드) 표시 | `'분류 목록'` |
| `'brand'` | 브랜드·키워드 선택 — `meta` 없음, 라벨 단일 표시 | `'브랜드·키워드 목록'` |
| `'generic'` | 기타 범용 (기본값) | `'목록 제안'` |

> variant는 렌더링 구조에 영향 없음 — `listLabel` 기본값과 의미론적 구분만 제어

### 12-1. 역할과 `CmsSimilarNameInput`과의 차이

| | `SuggestPicker` | `CmsSimilarNameInput` |
|---|---|---|
| 데이터 소스 | **페이지 로드 시 props로 전달된 로컬 옵션** | DB 비동기 검색 (RPC 호출) |
| 사용 목적 | `<select>` 대체 — 드롭다운 목록 선택 | 유사 이름 중복 방지 제안 |
| 검색 위치 | 클라이언트 메모리 필터링 | 서버 쿼리 |
| 대표 사례 | 카테고리 선택, 조합그룹 선택, 분류 선택 | 상품명 입력, 브랜드 입력 |

### 12-2. 데이터 타입

```typescript
// src/lib/types/suggest-picker.ts
export interface SuggestPickerOption {
  id:     string        // 선택값 (FK, code 등)
  label:  string        // 표시 텍스트
  meta?:  string[]      // 보조 정보 (카테고리·설명 등)
}
...
| Prop | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `selectedId` | `string \| null` (bindable) | `null` | 선택된 항목 id |
| `options` | `SuggestPickerOption[]` | `[]` | 표시할 옵션 목록 |
| `id` / `name` | `string` | — | 폼 필드 식별 |
| `placeholder` | `string` | `''` | 플레이스홀더 |
| `required` | `boolean` | `false` | 필수 여부 |
| `minChars` | `number` | `0` | 최소 입력 글자 (`0` = 포커스 시 전체 목록) |
| `variant` | `SuggestPickerVariant` | `'generic'` | `'category'` / `'brand'` / `'generic'` |
...
<script lang="ts">
  ...
  const categoryOptions: SuggestPickerOption[] = data.categories.map((c) => ({
    id:    c.id,
    label: CAT_LABELS[c.code] ?? c.name,
    meta:  [c.name, c.code],   // 2줄 보조정보 표시
  }))
</script>

<label for="cat-picker">카테고리</label>
<SuggestPicker
  id="cat-picker"
  variant="category"
...
>
  {#snippet field(c)}
    <input type="text" class="f-input" id={c.id}
      placeholder={c.placeholder} value={c.value}
...
### 12-5. ⛔ 명시적 UI × 규칙 — 이 컴포넌트에서 절대 금지

```
❌ <select> 태그 사용 — SuggestPicker로 대체해야 함
   이유: 브라우저 기본 select UI가 디자인 시스템 토큰 미적용
   → 모든 드롭다운 목록 선택에는 SuggestPicker 사용

❌ field 스니펫 없이 직접 input 렌더 — field 스니펫 + 페이지 .f-input 필수
   이유: Svelte scoped CSS가 컴포넌트 내부 input에 적용 안 됨

❌ bind:selectedId 없이 options만 전달 — selectedId가 없으면 선택 상태 소실
   이유: 폼 제출 시 선택값 전달 불가

❌ minChars를 1 이상으로 설정 (분류 선택 UI에서)
   이유: 포커스 시 전체 목록 표시가 UX 원칙 (minChars=0 기본값 유지)

❌ 선택 제안 레이어를 page-level z-index 아래 렌더링
   이유: 레이어 z-index: 40 고정 — 상위 overflow:hidden 요소 안에 넣지 말 것
   → 폼 카드 외부에서 Portal 방식 대신 카드 overflow: visible 전환
```

### 12-6. UI·접근성 규격 (컴포넌트 내장)

```
레이어 스타일:
  background: var(--cs-white)
  border: 1.5px solid rgba(59,47,138,0.2)   /* 보라 tint 보더 */
  border-radius: 10px                        /* --cms-radius-sm */
  box-shadow: 0 8px 24px rgba(16,11,50,0.12)
  max-height: 280px / overflow-y: auto
  position: absolute top: calc(100% + 4px)   /* 인풋 하단 4px */

아이템 스타일:
  padding: 10px 16px
  border-bottom: 1px solid var(--cs-lilac)
  label: --text-pc-body-14 / --cs-text
  meta:  --text-pc-script-12 / --cs-text-light, gap: 6px
...
blur 딜레이:
  onblur: setTimeout(closeSuggest, 150)   ← onmousedown: preventDefault()와 쌍 필수
```

### 12-7. 재활용 체크리스트 (GATE C)

```
[ ] <select> 대신 SuggestPicker (공통) 사용? import: $lib/components/common/SuggestPicker.svelte
[ ] bind:selectedId 필수 전달?
[ ] field 스니펫에서 .f-input 직접 렌더?
[ ] minChars=0 (분류 선택 시)?
[ ] 레이어 부모에 overflow: visible 확인?
[ ] aria-autocomplete / aria-expanded / aria-controls 전달?
[ ] 선택 후 부가 필드 업데이트는 onselect/oninput에서만?
```
...
❌ 드롭다운 목록 선택에 `<select>` 사용 — `SuggestPicker` (공통) 재사용 필수 (Section 12)
❌ position: fixed 남용 — 레이아웃 컨텍스트 오염 주의
❌ SuggestPicker field 스니펫 없이 사용 — field 스니펫 + .f-input 필수
❌ SuggestPicker를 overflow:hidden 부모 안에 배치 — z-index 40 레이어 노출 불가
❌ 구경로 `CmsSuggestPicker` / `$lib/types/cms-suggest-picker` 신규 작성 — 공통 경로로만 작성
